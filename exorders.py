...
ID = form.getvalue("id")
q = """SELECT orders.id,useregister.firstname,useregister.lastname,useregister.address ,useregister.phno,orders.proname,orders.quan,orders.size,orders.rate,orders.card,orders.cardno,orders.exdate,orders.status FROM orders INNER JOIN useregister ON orders.userid = useregister.id where orders.status="Confirm" and orders.shopid="%s" """%ID
cur.execute(q)
rec = cur.fetchall()
