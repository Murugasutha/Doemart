...
q = """SELECT useregister.firstname,useregister.lastname,useregister.address ,useregister.phno,orders.proimg,orders.proname,orders.quan,orders.size,orders.rate,orders.status,orders.shopid,orders.id FROM orders INNER JOIN useregister ON orders.userid = useregister.id where orders.userid="%s" AND orders.status="Received" """ % ID
cur.execute(q)
rec = cur.fetchall()
